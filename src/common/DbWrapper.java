...
public class DbWrapper{
	protected static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(DbWrapper.class);
	private String Url;
	...
	public DbWrapper(){
		...
		try {
			...
			ds = (DataSource) context.lookup(db.equals("oracle") ? "java:jboss/datasources/OracleClient" : "java:comp/env/jdbc");
			return;
		...
		DataSourceClassName = configFile.getProperty("POOLCLASSNAME",db.equals("oracle") ? "oracle.jdbc.pool.OracleDataSource" : "com.mysql.jdbc.jdbc2.optional.MysqlDataSource");
	...
	public DbWrapper(String url, String username, String password){
		...
		configFile.setProperty("user", Username);
	...
	public DbWrapper(Properties properties){
		Driver = properties.getProperty("DRIVER","");
		DataSourceClassName = properties.getProperty("POOLCLASSNAME","oracle.jdbc.pool.OracleDataSource");
		Url = properties.getProperty("connection_url","jdbc:oracle:thin:@//192.168.137.10:1521/LOCAL");
		Username = properties.getProperty("user","ADMDBMF");
		Password = properties.getProperty("password","ADMDBMF");
		Crypt = properties.getProperty("CRYPT",Crypt);
		Init();
	...
	public void SetParams(PreparedStatement stm, Object... params) throws SQLException{
		...
		for(int i = 0; i < params.length; i++){
			try{
				if(params[i] instanceof InOutParameter){
					InOutParameter param = (InOutParameter)params[i];
					if((param.direction & 1) > 0){ //input
						switch(param.getType()){
							case java.sql.Types.NULL: //unknown
								stm.setObject(i + 1, param.getValue());
								break;
							case java.sql.Types.ARRAY:
								//oracle.sql.ARRAY
								stm.setArray(i + 1,createArray(stm.getConnection(),(Object[])param.getValue(), param.getName()));
								break;
							case java.sql.Types.CLOB:
								//oracle.sql.Types.CLOB
								...
								if(param.getValue() instanceof Reader){
									Reader reader = (Reader)param.getValue();
									stm.setClob(i + 1, reader);
									break;
								}
							case java.sql.Types.BLOB:
								//oracle.sql.Types.BLOB
								if(param.getValue() instanceof String){
									byte[] value = ((String)param.getValue()).getBytes(); 
									InputStream reader = new ByteArrayInputStream(value);
									stm.setBinaryStream(i + 1, reader, value.length);
									break;
								}
								if(param.getValue() instanceof InputStream){
									InputStream reader = (InputStream)param.getValue();
									stm.setBlob(i + 1, reader);
									break;
								}
							default :
					...
					if((param.direction & 2) > 0){ //output
						//Logger.LogServer(param.toString() + "***" + param.getType());
						if(param.name == null || param.name.length() == 0)
							((CallableStatement) stm).registerOutParameter(i + 1, param.getType());
						else
							((CallableStatement) stm).registerOutParameter(i + 1, param.getType(), param.name);
					}
...
/*
	public int QueryUpdate(String sql,Object... params){
		InOutParameter[] pars = new InOutParameter[params.length];
		for(int i = 0 ; i < params.length; i++){
			if(params[i] instanceof InOutParameter){
				pars[i] = (InOutParameter)params[i];
			}else{
				if(params[i] == null) 
					pars[i] = new InOutParameter("");
				else
					pars[i] = new InOutParameter(params[i]);
			}
		}
		return QueryUpdate(sql, pars);
	}
*/	
	public int QueryUpdate(String sql,Object... params){
		Connection con = null;
		...
		try{
			...
			for(int i = 0; i < params.length; i++){
				if(params[i] instanceof InOutParameter){
					InOutParameter param = (InOutParameter)params[i];
					if((param.direction & 2) > 0){
						Object obj = ((CallableStatement) stm).getObject(i + 1);
						if(obj instanceof java.sql.Array)
							param.setValue(((java.sql.Array)obj).getArray());
						else
							param.setValue(obj);
					}
				}else{
		...
		try{
            ...
            while (rs.next() ) {
            	...
            	for(int i = 0; i < numColumns; i++){
                	...
                	if(obj instanceof Clob){
                		Clob clob = (Clob)obj;
                		putColumn(row,rsmd.getColumnName(i + 1), getClobString(clob));
                	}else if(obj instanceof Blob){
                		Blob blob = (Blob)obj;
                		putColumn(row,rsmd.getColumnName(i + 1), getBlobBytes(blob));
                	}else
                		putColumn(row,rsmd.getColumnName(i + 1), obj);
            	}
            	rows.add(row);
            }
		}catch(Exception e){
	...
	private void putColumn(DataRow row, String column, Object value){
		if(row.containsKey(column)){
			for(int i = 1; ; i++){
				if(!row.containsKey(column + i)){
